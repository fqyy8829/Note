...
&emsp;&emsp;还可以使用<font color=red> *__list()函数__* </font>将其它类型转换成列表类型， 但凡能被<font color=red> *__for循环遍历的数据类型都可以传给 list() 转换成列表类型__* </font>，list()会跟for循环一样遍历出数据类型中包含的每一个元素然后放到列表中：
...
+ <font color=green>**min：** </font>求数字列表中的最小的数
+ <font color=green>**max：** </font>求数字列表中的最大的数
+ <font color=green>**sum：** </font>求数字列表中的所有数字的和
...
&emsp;&emsp;列表提供了<font color=red> **in** </font>和<font color=red> **not in** </font>来判断一个元素在不在列表中：

+ <font color=green>**a in list：** </font> a在list列表中，在则返回True，否则返回False
+ <font color=green>**a not in list：** </font> a是不是不在list列表中，不在则返回True，否则返回False
